...
public:
  ...
  struct UlSCHConfig
  {
      uint8_t maxHARQTx;            // [1,2,3,4,5,6,7,8,10,12,16,20,24,28]
  ...
  struct PhrConfig
  {
      uint16_t  periodicPHRTimer;  // in SF: [10,20,50,100,200,500,1000,infinite]
      uint16_t  prohibitPHRTimer;  // in SF: [0,10,20,50,100,200,500,1000]
      uint8_t   dlPathlossChange;  // in db: [1,3,6,infinite]
  };

  struct MACMainConfig
  {
      UlSCHConfig   ulSCHConfig;                    // Optional
      DrxConfig_s   drxConfig;                      // Optional
      uint32_t      timeAlignmentTimerDedicated;    // [500,750,1280,1920,2560,5120,10240,infinite]
      PhrConfig     phrConfig;                      // Optional
  ...
  struct PcchConfig
  {
      uint16_t    defaultPagingCycle;           // default DRX cycle: [32, 64, 128, 256] in rf (radio frame),  rf32 means 320ms
      float       nb;                           // In T: [ 4, 2, 1, 1/2, 1/4, 1/8, 1/16, 1/32]
  };

  struct BcchConfig
  {
      uint16_t    modificationPeriodCoeff;      // [2,4,8,16]
  };
