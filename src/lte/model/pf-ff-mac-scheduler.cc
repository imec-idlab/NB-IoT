  ...
  for (it = m_rlcBufferReq.begin (); it != m_rlcBufferReq.end (); it++)
    {
      if (((*it).first.m_rnti == rnti) && (((*it).second.m_rlcTransmissionQueueSize > 0)
        ...
        {
          std::cout << "m_rnti:" << (*it).first.m_rnti << " m_rlcTransmissionQueueSize:" << (*it).second.m_rlcTransmissionQueueSize << " m_rlcRetransmissionQueueSize:" << (*it).second.m_rlcRetransmissionQueueSize << " m_rlcStatusPduSize:" << (*it).second.m_rlcStatusPduSize << "\n";
          lcActive++;
        }
...
PfFfMacScheduler::DoSchedDlTriggerReq (const struct FfMacSchedSapProvider::SchedDlTriggerReqParameters& params)
{
  ...
  /*std::cout << "Initial rbgMap: ";
  index_pouria = 0;
  for (std::vector<bool>::const_iterator j = rbgMap.begin(); j != rbgMap.end(); ++j, ++index_pouria)
    std::cout << index_pouria << ":" << *j << ' ';
  std::cout << "\n";*/
  ...
  for (itRach = m_rachList.begin (); itRach != m_rachList.end (); itRach++)
    {
      //std::cout << "m_ulGrantMcs: " << (int)m_ulGrantMcs << " m_ulBandwidth: " << (int)m_cschedCellConfig.m_ulBandwidth << " m_estimatedSize:" << (*itRach).m_estimatedSize << "\n";
      //std::cout << "m_amc->GetTbSizeFromMcs (m_ulGrantMcs, m_cschedCellConfig.m_ulBandwidth): " << m_amc->GetTbSizeFromMcs (m_ulGrantMcs, m_cschedCellConfig.m_ulBandwidth) << "\n";
      ...
      //std::cout << "1-m_estimatedSize: " << (*itRach).m_estimatedSize  << " rbStart: " << rbStart << " rbLen: " << rbLen << " ffrRbStartOffset: " << ffrRbStartOffset  << " maxContinuousUlBandwidth:" << (int)maxContinuousUlBandwidth << " tbSizeBits:" << tbSizeBits << "\n";
      while ((tbSizeBits < (*itRach).m_estimatedSize) && (rbStart + rbLen < (ffrRbStartOffset + maxContinuousUlBandwidth)))
        {
          ...
          //std::cout << "2-m_estimatedSize: " << (*itRach).m_estimatedSize  << " rbStart: " << rbStart << " rbLen: " << rbLen << " ffrRbStartOffset: " << ffrRbStartOffset  << " maxContinuousUlBandwidth:" << (int)maxContinuousUlBandwidth << " tbSizeBits:" << tbSizeBits << "\n";
        }
      if (tbSizeBits < (*itRach).m_estimatedSize)
        {
          // no more allocation space: finish allocation
          //std::cout << "no more allocation space: finish allocation \n";
          //std::cout << "3-m_estimatedSize: " << (*itRach).m_estimatedSize  << " rbStart: " << rbStart << " rbLen: " << rbLen << " ffrRbStartOffset: " << ffrRbStartOffset  << " maxContinuousUlBandwidth:" << (int)maxContinuousUlBandwidth << " tbSizeBits:" << tbSizeBits << "\n";
          break;
        }
      ...
      if (rbgMap.at (i) == false)
        {
          ...
          double rcqiMax = 0.0;
          std::map <uint16_t, pfsFlowPerf_t>::iterator itStats2;
          for (itStats2 = m_flowStatsDl.begin (); itStats2 != m_flowStatsDl.end (); itStats2++)
          {
            NS_LOG_INFO (this << " UE total bytes " << (*itStats2).second.totalBytesTransmitted);
            NS_LOG_INFO (this << " UE average throughput " << (*itStats2).second.lastAveragedThroughput);
            NS_LOG_INFO (this << " RNTI " << (*itStats2).first);
            //m_flowStatsDl.insert (std::pair<uint16_t, pfsFlowPerf_t> (params.m_rnti, flowStatsDl));
          }
          for (it = m_flowStatsDl.begin (); it != m_flowStatsDl.end (); it++)
            {
              ...
              if (itCqi == m_a30CqiRxed.end ())
              ...
              else
                {
                  ...
                  /*std::cout << "sbCqi";
                  for(int j = 0; j < (int)sbCqi.size (); j++)
                    std::cout << "[" << j << "]:" << (int)sbCqi.at(j);
                  std::cout << "\n";*/
                }
                      ...
                      if (rcqi > rcqiMax)
                        {
                          rcqiMax = rcqi;
                          itMax = it;
                          NS_LOG_INFO (this << " RNTI " << (*it).first << " tMax = it");
                        }
                    }
                }   // end if cqi
              ...
              if (itMap == allocationMap.end ())
                {
                  ...
                  allocationMap.insert (std::pair <uint16_t, std::vector <uint16_t> > ((*itMax).first, tempMap));
                   std::cout << "insert into allocationMap for RNTI:" << (*itMax).first;
                  for(int j = 0; j < (int)tempMap.size(); j++)
                    std::cout << " " << tempMap[j];
                  std::cout << "\n";
                }
              else
                {
                  ...
                  std::cout << "second.push_back::" << i<< "\n";
                }
              NS_LOG_INFO (this << " UE assigned " << (*itMax).first);
            }
        } // end for RBG free
    } // end for RBGs
  std::cout << "rbgMap After: ";
  index_pouria = 0;
  for (std::vector<bool>::const_iterator j = rbgMap.begin(); j != rbgMap.end(); ++j, ++index_pouria)
    std::cout << index_pouria << ":" << *j << ' ';
  std::cout << "\n";
  ...
  std::map <uint16_t, std::vector <uint16_t> >::iterator itMapTemp = allocationMap.begin ();
  while (itMapTemp != allocationMap.end ())
  {
    std::cout << "RNTI:"  << (*itMapTemp).first << ";;;";
    for(int j = 0; j < (int)(*itMapTemp).second.size (); j++)
      std::cout << " "  << (*itMapTemp).second.at(j);
    std::cout << "\n";
    itMapTemp++;
  }
          ...
          if (it == m_a30CqiRxed.end ())
            {
              ...
              //std::cout << "rnti:" << (int)rnti << " m_sbMeasResult:"<< (int)params.m_cqiList.at (i).m_sbMeasResult.m_higherLayerSelected.at (0).m_sbCqi.at (0) << "\n";
              //NS_ASSERT(0);
            }
          else
            {
...
PfFfMacScheduler::DoSchedUlTriggerReq (const struct FfMacSchedSapProvider::SchedUlTriggerReqParameters& params)
{
  ...
  /*std::cout << "Initial rbAllocatedNum: " << rbAllocatedNum << " rbMap.size:" << rbMap.size() << "\n";
  std::cout << "Initial rbMap: ";
  for (std::vector<bool>::const_iterator j = rbMap.begin(); j != rbMap.end(); ++j)
    std::cout << *j << ' ';
  std::cout << "\n";*/
      ...
      if (rbgAllocationMap.at (i) != 0)
        {
          ...
          /*std::cout << "rbgAllocationMap: ";
          for (std::vector<uint16_t>::const_iterator j = rbgAllocationMap.begin(); j != rbgAllocationMap.end(); ++j)
            std::cout << *j << ' ';
          std::cout << "\n";
          std::cout << "rbMap: ";
          for (std::vector<bool>::const_iterator j = rbMap.begin(); j != rbMap.end(); ++j)
                  ...
                  for (int j = dci.m_rbStart; j < dci.m_rbStart + dci.m_rbLen; j++)
                    {
                      ...
                      for (std::vector<bool>::const_iterator j = rbMap.begin(); j != rbMap.end(); ++j)
                        std::cout << *j << ' ';
                      std::cout << "\n";
                      std::cout << "rbgAllocationMap4 Before: ";
                      for (std::vector<uint16_t>::const_iterator j = rbgAllocationMap.begin(); j != rbgAllocationMap.end(); ++j)
                        std::cout << *j << ' ';
                      std::cout << "\n";*/
                      rbgAllocationMap.at (j) = dci.m_rnti;
                      /*std::cout << "rbgAllocationMap4 After: ";
                      for (std::vector<uint16_t>::const_iterator j = rbgAllocationMap.begin(); j != rbgAllocationMap.end(); ++j)
                        std::cout << *j << ' ';
                      std::cout << "\n";*/
                      NS_LOG_INFO ("\tRB " << j);
  ...
  //if (rbPerFlow < 3)
  //  {
      rbPerFlow = 1;//3;  // at least 3 rbg per flow (till available resource) to ensure TxOpportunity >= 7 bytes
  //  }
              ...
              for (uint16_t j = rbAllocated; j < rbAllocated + rbPerFlow; j++)
                {
                  rbMap.at (j) = true;
                  //=========================================================================================================
                  /*std::cout << "rbgAllocationMap3 Before: ";
                  for (std::vector<uint16_t>::const_iterator j = rbgAllocationMap.begin(); j != rbgAllocationMap.end(); ++j)
                    std::cout << *j << ' ';
                  std::cout << "\n";*/
                  //=========================================================================================================
                  ...
                  //=========================================================================================================
                  /*std::cout << "rbgAllocationMap3 After: ";
                  for (std::vector<uint16_t>::const_iterator j = rbgAllocationMap.begin(); j != rbgAllocationMap.end(); ++j)
                    std::cout << *j << ' ';
                  std::cout << "\n";
                  std::cout << "rbMap3: ";
    ...
    case UlCqi_s::PUSCH:
      {
        ...
        //====================================================================================================
        /*std::cout << "m_allocationMaps: ";
        std::map <uint16_t, std::vector <uint16_t> >::iterator itMapTemp = m_allocationMaps.begin ();
        while (itMapTemp != m_allocationMaps.end ())
        {
          std::cout << "RNTI:"  << (*itMapTemp).first << ";;;";
          for(int j = 0; j < (int)(*itMapTemp).second.size (); j++)
            std::cout << " "  << (*itMapTemp).second.at(j);
  ...
  if (it != m_rlcBufferReq.end ())
    {
      ...
      if (((*it).second.m_rlcStatusPduSize > 0) && (size >= (*it).second.m_rlcStatusPduSize))
      ...
      else if ((*it).second.m_rlcTransmissionQueueSize > 0)
        {
          ...
          uint32_t oldQueue = (*it).second.m_rlcTransmissionQueueSize;
          if ((*it).second.m_rlcTransmissionQueueSize <= size - rlcOverhead)
            {
              (*it).second.m_rlcTransmissionQueueSize = 0;
              std::cout << "rnti: " << rnti << " old m_rlcTransmissionQueueSize: " << oldQueue << " new m_rlcTransmissionQueueSize: " << (*it).second.m_rlcTransmissionQueueSize << "\n";
            }
          else
            {
