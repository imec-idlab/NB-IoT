  ...
  do
    {
      ...
      int N_rb_dl = m_cschedCellConfig.m_dlBandwidth;
      int N_prb = rbgPerTb;
      newDci.m_repetitionNumber = repetitionCounter++;
      if((N_prb - 1) <= (N_rb_dl/2))
      {
        riv = N_rb_dl * (N_prb - 1) + rb_start;
#ifdef DCI_BW_DEBUG
...
RrFfMacScheduler::DoSchedUlTriggerReq (const struct FfMacSchedSapProvider::SchedUlTriggerReqParameters& params)
{
  ...
  std::cout << " UL - Frame no. " << (params.m_sfnSf >> 4) << " subframe no. " << (0xF & params.m_sfnSf) << "\n";
  RefreshUlCqiMaps ();
          ...
          if (params.m_ulInfoList.at (i).m_receptionStatus == UlInfoListElement_s::NotOk)
            {
              ...
              int N_prb = rbPerFlow;
              uldci.m_repetitionNumber = repetitionCounter++;
              if((N_prb - 1) <= (N_rb_dl/2))
              {
                riv = N_rb_dl * (N_prb - 1) + rb_start;
#ifdef DCI_BW_DEBUG
                std::cout << "[RfFF:uldci]riv: " << riv << " N_prb:" << N_prb << " N_rb_dl:" << N_rb_dl << " rb_start:" << rb_start << " m_repetitionNumber " << uldci.m_repetitionNumber << "\n";
#endif
              }
              else
              {
...
//HERE CHECK THE FIXED RB OF EACH RNTI
      if(RBno.at (uldci.m_rnti) != 200 )
      {
        ...
        if (rbMap.at(RBno.at (uldci.m_rnti))==true)        
        {
          std::cout << "fixed RB " << RBno.at (uldci.m_rnti) << " in use by " << rbgAllocationMap.at (RBno.at (uldci.m_rnti)) <<  "not available for " << uldci.m_rnti << "\n";  
          if (uldci.m_rnti < min)
            min = uldci.m_rnti;
          it++;
          if (it == m_ceBsrRxed.end ())
     ...
     else
     {
      ...
      if (itCqi == m_ueCqi.end ())
      ...
      else
        {
          ...
          double minSinr =  200; //(*itCqi).second.at (rbStartTemp/*uldci.m_rbStart*/);
          //NS_LOG_INFO (this << " minSinr " << (double)minSinr << " m_rbStart " << (int) uldci.m_rbStart << " rbStartTemp " << (int)rbStartTemp << " m_rbLen " << (int) uldci.m_rbLen << " flows " << nflows << " m_ulBandwidth:" << (int)m_cschedCellConfig.m_ulBandwidth);
          //std::cout << this << " minSinr " << (double)minSinr << " m_rbStart " << (int) uldci.m_rbStart << " rbStartTemp " << (int)rbStartTemp << " m_rbLen " << (int) uldci.m_rbLen << " flows " << nflows << " m_ulBandwidth:" << (int)m_cschedCellConfig.m_ulBandwidth << "\n";
          for (uint16_t i = rbStartTemp/*uldci.m_rbStart*/; i < m_cschedCellConfig.m_ulBandwidth/*uldci.m_rbStart + uldci.m_rbLen*/; i++) {
              if ((*itCqi).second.at (i) < minSinr && rbMapTemp.at (i) == false)
                {
          ...
          if (cqi == 0 || m_amc->GetMcsFromCqi (cqi) <= 2)
            {
              ...
              /*for (uint16_t i = 0; i < 100; i++)
               {
                     std::cout << " " << (double)(*itCqi).second.at (i);
               }
              std::cout << "\n\n";
              for (uint16_t i = 0; i < 100; i++)
              {
                if(rbMapTemp.at (i) == false)
                  std::cout << " " << (double)(*itCqi).second.at (i);
              }
              std::cout << "\n";
              for (std::vector<bool>::iterator it = rbMapTemp.begin (); it != rbMapTemp.end (); it++)
            ...
            {
              /*std::cout << " UE not discared for CQI!=0, RNTI " << uldci.m_rnti << " minSinr:" << minSinr << " uldci.m_rbStart:" << (int)uldci.m_rbStart << " uldci.m_rbLen:" << (int)uldci.m_rbLen <<"\n";
              for (uint16_t i = 0; i < m_cschedCellConfig.m_ulBandwidth; i++)
               {
                     std::cout << " " << (double)(*itCqi).second.at (i);
               }
              ...
              std::cout << "\n";*/
              //std::cout << " UE Not discared for CQI!=0, RNTI " << uldci.m_rnti << " minSinr:" << minSinr << " s: " << s << " uldci.m_rbStart:" << (int)uldci.m_rbStart << " uldci.m_rbLen:" << (int)uldci.m_rbLen <<"\n";
              //std::cout << " UE Not discared for CQI!=0, RNTI" << uldci.m_rnti  << " cqi:" << cqi << " mcs:" << m_amc->GetMcsFromCqi (cqi) << " tbs: " << (m_amc->GetTbSizeFromMcs (m_amc->GetMcsFromCqi (cqi), rbPerFlow) / 8) << " rbStart:" << (int)uldci.m_rbStart << " rbLen:" << (int)uldci.m_rbLen << "\n";
            }
      ...
      if((N_prb - 1) <= (N_rb_dl/2))
      {
        riv = N_rb_dl * (N_prb - 1) + rb_start;
#ifdef DCI_BW_DEBUG
        std::cout << "[RfFF:uldci]riv: " << riv << " N_prb:" << N_prb << " N_rb_dl:" << N_rb_dl << " rb_start:" << rb_start << " m_repetitionNumber " << uldci.m_repetitionNumber <<  " RNTI " << uldci.m_rnti << "\n";
#endif
      }
      else
      {
