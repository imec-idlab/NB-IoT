...
LteEnbMac::DoSubframeIndication (uint32_t frameNo, uint32_t subframeNo)
{
  ...
  //std::cout << " EnbMac - frame " << frameNo << " subframe " << subframeNo << "\n";
  ...
  // EndMAC should send dlCqi, once every two slot?
  NS_LOG_FUNCTION (this << " Send Dl-CQI info to the scheduler ====================================================");
...
//     }
  ...
  std :: cout << "RXED AT ENB RNTI "<<  rnti << " TIME " << Simulator::Now().GetSeconds() << "\n";
...
LteEnbMac::DoReportBufferStatus (LteMacSapProvider::ReportBufferStatusParameters params)
{
  ...
  //std::cout << "m_rlcTransmissionQueueSize: " << req.m_rlcTransmissionQueueSize << "\n";
  ...
  for (  uint32_t i  = 0; i < ind.m_dciList.size (); i++ )
    {
      m_ulScheduling (m_frameNo, m_subframeNo, ind.m_dciList.at (i).m_rnti,
                      ind.m_dciList.at (i).m_mcs, ind.m_dciList.at (i).m_tbSize, ind.m_dciList.at (i).m_scindication,ind.m_dciList.at (i).m_repetitionNumber);
    }
}
  ...
  for (uint8_t layer = 0; layer < params.m_harqStatus.size (); layer++)
    {
      if (params.m_harqStatus.at (layer) == DlInfoListElement_s::ACK)
      ...
      else if (params.m_harqStatus.at (layer) == DlInfoListElement_s::NACK)
        {
          ...
          std::cout << Now().GetSeconds() << " HARQ-NACK UE " << params.m_rnti << " harqId " << (uint16_t)params.m_harqProcessId << " layer " << (uint16_t)layer << "\n";
        }
      else
        {
