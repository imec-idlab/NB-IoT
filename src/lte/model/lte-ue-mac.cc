      ...
      if (dci.m_ndi == 1)
        {
          ...
          //std::cout << this << " UE " << m_rnti << ": UL-CQI notified TxOpportunity of " << dci.m_tbSize << " => " << bytesPerActiveLc << " bytes per active LC" << " statusPduMinSize " << statusPduMinSize << "\n";
                  ...
                  if ((statusPduPriority) && ((*itBsr).second.statusPduSize == statusPduMinSize))
                    {
                      ...
                      //std::cout << this << "\t" << bytesPerActiveLc << " send  " << (*itBsr).second.statusPduSize << " status bytes to LC " << (uint32_t)(*it).first << " statusQueue " << (*itBsr).second.statusPduSize << " retxQueue " << (*itBsr).second.retxQueueSize << " txQueue" <<  (*itBsr).second.txQueueSize << "\n"; 
                      (*itBsr).second.statusPduSize = 0;
                      break;
                    }
                  else
                    {
                      ...
                      //std::cout << this << "\t" << bytesPerActiveLc << " bytes to LC " << (uint32_t)(*it).first << " statusQueue " << (*itBsr).second.statusPduSize << " retxQueue " << (*itBsr).second.retxQueueSize << " txQueue " <<  (*itBsr).second.txQueueSize << "\n";
      ...
      else 
        {
         ...
         Ptr<PacketBurst> pb = m_miUlHarqProcessesPacket.at (m_harqProcessId);
          //Ptr<PacketBurst> pb = CreateObject <PacketBurst> ();
          //std::cout << Now().GetSeconds() << " UE MAC RETX HARQ OUT " << (uint16_t)m_harqProcessId <<"\n";
          for (std::list<Ptr<Packet> >::const_iterator j = pb->Begin (); j != pb->End (); ++j)
            {
              Ptr<Packet> pkt = (*j)->Copy ();
             // std::cout << Now().GetSeconds() << " UE MAC RETX HARQ IN " << (uint16_t)m_harqProcessId <<"\n";
  ...
  for (uint16_t i = 0; i < m_miUlHarqProcessesPacketTimer.size (); i++)
    {
      if (m_miUlHarqProcessesPacketTimer.at (i) == 0)
        {
          if (m_miUlHarqProcessesPacket.at (i)->GetSize () > 0 && m_subframeNo%10==0)
            {
              // timer expired: drop packets in buffer for this process
             // std::cout << Now().GetSeconds() <<" HARQ Proc Id" << i << " packets buffer expired" << "\n";
