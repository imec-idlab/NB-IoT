...
{
  NS_LOG_FUNCTION (this);
  //std::cout << "LteEnbPhy::DoSendMacPdu" << "\n";
  SetMacPdu (p);
}
...
LteEnbPhy::PhyPduReceived (Ptr<Packet> p)
{
 ...
 std::cout << "Time " << Simulator::Now().GetSeconds()  << "ENBPhy::PhyPduReceived" << "\n";
...
LteEnbPhy::StartFrame (void)
{
  ...
  //std::cout << "LtePhy::GetControlMessages.size():" << m_controlMessagesQueue.size() << " m_controlMessagesQueue.at (0).size (): "  << m_controlMessagesQueue.at (0).size () << " m_controlMessagesQueue.at (1).size (): "  << m_controlMessagesQueue.at (1).size () <<"\n";
  //m_dlControlMessagesQueue.at (0).push_back (mibMsg);
...
LteEnbPhy::StartSubFrame (void)
{
  ...
  if ((m_nrSubFrames == 6) && ((m_nrFrames % 2) == 1))
    {
      ...
      //m_dlControlMessagesQueue.at (0).push_back (msg);
    }
  ...
  //std ::cout << "Time "<< Simulator::Now().GetSeconds() << " Calling DequeueUlDci " << "\n";
  ...
  for (dciIt = uldcilist.begin (); dciIt!=uldcilist.end (); dciIt++)
    {
      ...
      if (it2 == m_ueAttached.end ())
        ...
        //for(int i=0; i < (*dciIt).GetDci().m_repetitionNumber;i++)
        {
         //std ::cout << "Time "<< Simulator::Now().GetSeconds() << " Calling AddExpectedTb " << (*dciIt).GetDci().m_repetitionNumber << "\n";
          m_uplinkSpectrumPhy->AddExpectedTb ((*dciIt).GetDci ().m_rnti, (*dciIt).GetDci ().m_ndi, (*dciIt).GetDci ().m_tbSize, (*dciIt).GetDci ().m_mcs, rbMap, (*dciIt).GetDci ().m_scindication,12, 0 /* always SISO*/, 0 /* no HARQ proc id in UL*/, 0 /*evaluated by LteSpectrumPhy*/, false /* UL*/, (*dciIt).GetDci().m_repetitionNumber);
          }
         // Simulator::Schedule(MilliSeconds(1), &LteSpectrumPhy::AddExpectedTb, this, (*dciIt).GetDci ().m_rnti, (*dciIt).GetDci ().m_ndi, (*dciIt).GetDci ().m_tbSize, (*dciIt).GetDci ().m_mcs, rbMap, 0 , 0 , 0 , false);
      ...
      while (it != ctrlMsg.end ())
        {
          ...
          if (msg->GetMessageType () == LteControlMessage::DL_DCI)
          ...
          else if (msg->GetMessageType () == LteControlMessage::UL_DCI)
            {
              ...
              dci = (*uldciMsg).GetDci ();
              Ptr<UlDciLteControlMessage> uldciMsg1= Create<UlDciLteControlMessage> ();
              int m_nooftti=1;

              m_nooftti=0;
              Simulator::Schedule (MilliSeconds(m_nooftti), // ctrl frame fixed to 3 symbols
                      &LteEnbPhy::QueueUlDci,
                      this,*uldciMsg);
              std::set <uint16_t>::iterator it2;
              ...
              if (it2 == m_ueAttached.end ())
              ...
              else
                {
                  ...
                  int scindication = (*uldciMsg).GetDci ().m_scindication;
                  int repetition = (*uldciMsg).GetDci ().m_repetitionNumber;
                  UlDciListElement_s uldci;
                  uldci = (*uldciMsg).GetDci ();
                  uldci.m_ndi =0;
                 ...
                 if (dci.m_scindication <= 11 && dci.m_scindication >=0 )
                  {          
                    m_nooftti = 9;
                  ...
                  {
                    (*uldciMsg1).SetDci(uldci);
                    Simulator::Schedule (MilliSeconds((i-1)*m_nooftti), // ctrl frame fixed to 3 symbols
                      &LteEnbPhy::QueueUlDci,
                      this,*uldciMsg1);
                    uldci.m_ndi = 1;
                  }
  ...
  /*std::list<Ptr<LteControlMessage> > ctrlMsg2 = GetDlControlMessages (temp_nrSubFrames);
  // process the current burst of control messages
  NS_LOG_FUNCTION (this << " LteEnbPhy::process the current burst of dl control messages==================================");
  if (ctrlMsg2.size () > 0)
    {
      std::list<Ptr<LteControlMessage> >::iterator it;
      m_dlDataRbMap.clear ();
      m_dlPowerAllocationMap.clear ();
      it = ctrlMsg2.begin ();
      while (it != ctrlMsg2.end ())
        {
          Ptr<LteControlMessage> msg = (*it);
          if (msg->GetMessageType () != LteControlMessage::DL_DCI)
          {
            NS_LOG_FUNCTION (this << " LteEnbPhy::LteControlMessage NOT DL_DCI==================================");
            NS_ASSERT(0);
          }
  ...
  ctrlMsg.merge (ctrlMsg2);*/
  NS_LOG_FUNCTION (this << " LteEnbPhy::SendControlChannels==================================");
  //std::cout << this << " LteEnbPhy::SendControlChannels ==================================\n";
...
{
  NS_LOG_FUNCTION (this << " eNB " << m_cellId << " start tx ctrl frame ccccccccccccccccccccccccccccccccccccccccccccccccc");
...
LteEnbPhy::SendDataChannels (Ptr<PacketBurst> pb)
{
  ...
  NS_LOG_LOGIC (this << " eNB start TX DATA ddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd");
...
//   for(int i=0;i<100;i++)
//     std::cout << LteFfConverter::fpS11dot3toDouble(ulcqi.m_ulCqi.m_sinr.at(i))<<" ";
// std::cout << "\n";
