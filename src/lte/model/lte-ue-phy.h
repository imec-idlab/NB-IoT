...
public:
  ...
  */
   ...
   */
   ...
   Ptr<SpectrumValue> CreateTxPowerSpectralDensity1 (bool srs=false);

  /**
   * \brief Set a list of sub channels to use in TX
   * \param mask a list of sub channels
   */
  void SetSubChannelsForTransmission (std::vector <int> mask, bool srs=false);
  /**
  ...
  */
  std::list<DlDciLteControlMessage> DequeueDlDci (void);
  void QueueDlDci (DlDciLteControlMessage m);


  /// For storing info on future receptions.
  std::vector< std::list<DlDciLteControlMessage> > m_dlDciQueue;
...
private:
  ...
  void QueueSubChannelsForReception(std::vector <int> rbMap);
  ...
  std::vector< std::vector <int> > m_subChannelsForReceptionQueue;
  ...
  // NEW PARAMETER FOR NB-IOT to represent # of subframes based on tone
  uint32_t m_nooftti;
  uint32_t m_scstart;
  uint32_t m_sfleft;
   uint32_t m_ttileft;
  uint32_t m_tone;
  uint32_t m_repetitionNumber;
  uint32_t m_nRetransmissions;
  uint32_t m_spacing;
  ...
  Ptr<PacketBurst>  m_RetransmitPacket; /// PACKET FOR RETRANSMISSION MULTITONEs

  std::vector <int> m_prevrb; 
  std::list<Ptr<LteControlMessage> > m_prevctrlMsg;
  bool m_srs;
    int64_t m_timestamp;
  /**
