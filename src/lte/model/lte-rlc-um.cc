...
LteRlcUm::ReassembleAndDeliver (Ptr<Packet> packet)
{
  ...
  if ( currSeqNumber != m_expectedSeqNumber )
    {
      expectedSnLost = true;
      NS_LOG_LOGIC ("ReassembleAndDeliver There are losses. Expected SN = " << m_expectedSeqNumber << ". Current SN = " << currSeqNumber);
      m_expectedSeqNumber = currSeqNumber + 1;
    }
  else
    {
      expectedSnLost = false;
      NS_LOG_LOGIC ("ReassembleAndDeliver No losses. Expected SN = " << m_expectedSeqNumber << ". Current SN = " << currSeqNumber);
      m_expectedSeqNumber++;
    }
  ...
  NS_LOG_LOGIC ("Framing Info = " << (uint16_t)framingInfo << " expectedSnLost " << expectedSnLost << "m_sdusBuffer.size () " << m_sdusBuffer.size ());
                  ...
                  switch (framingInfo)
                    {
                      case (LteRlcHeader::NO_FIRST_BYTE | LteRlcHeader::LAST_BYTE):
                              ...
                              */
                              ...
                              m_sdusBuffer.pop_front ();
                              NS_LOG_LOGIC ("NO_FIRST_BYTE but LAST_BYTE ReceivePdcpPdu" );
                      ...
                      case (LteRlcHeader::NO_FIRST_BYTE | LteRlcHeader::NO_LAST_BYTE):
                              ...
                              */
                              if ( m_sdusBuffer.size () == 1 )
                                {
                                   NS_LOG_LOGIC ("if NO_FIRST_BYTE NO_LAST_BYTE NO ReceivePdcpPdu" );
                                  m_keepS0->AddAtEnd (m_sdusBuffer.front ());
                                  m_sdusBuffer.pop_front ();
                                }
                              else // m_sdusBuffer.size () > 1
                                {
                                  ...
                                  * Deliver (Kept)S0 + SN
                                  */
                                   NS_LOG_LOGIC ("else -NO_FIRST_BYTE NO_LAST_BYTE YES ReceivePdcpPdu" );
                                  m_keepS0->AddAtEnd (m_sdusBuffer.front ());
                                  m_sdusBuffer.pop_front ();
                      ...
                      case (LteRlcHeader::FIRST_BYTE | LteRlcHeader::LAST_BYTE): NS_LOG_LOGIC ("LAST_BYTE" );
                      case (LteRlcHeader::FIRST_BYTE | LteRlcHeader::NO_LAST_BYTE):NS_LOG_LOGIC ("NO_LAST_BYTE" );
                      default:
                              /**
                                * ERROR: Transition not possible
                                */
