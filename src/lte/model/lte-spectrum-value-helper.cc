  ...
  if (it != g_lteSpectrumModelMap.end ())
  ...
  else
    {
     ...
     // std::cout << "fc" << fc << "\n";
      ...
      double f = fc - (txBandwidthConfiguration * SPACING / 2.0);

      Bands rbs;
      BandInfo rb; 
uint32_t numrb ;
      for (numrb = 0; numrb < txBandwidthConfiguration*NSC; ++numrb)
        {
          ...
          f += SPACING / 2.0;
          rb.fh = f;
           //std::cout << "numrb "<< numrb << " txBandwidthConfiguration*4 " <<  txBandwidthConfiguration*4 <<" SIZE OF RBS" << rbs.size() * sizeof(rb)* 3 << " BYTES \n\n\n";
          rbs.push_back (rb);
        }
        //std::cout << "SIZE OF RBS" << rbs.size() * sizeof(rb)* 3 << " BYTES \n\n\n";
...
Ptr<SpectrumValue> 
LteSpectrumValueHelper::CreateTxPowerSpectralDensity (uint32_t earfcn, uint8_t txBandwidthConfiguration, double powerTx, std::vector <int> activeRbs, uint32_t tone, uint32_t sc_start, uint32_t spacing)
{
  ...
  txBandwidthConfiguration=1;

  // powerTx is expressed in dBm. We must convert it into natural unit.
  double powerTxW = std::pow (10., (powerTx - 30) / 10);
  //std::cout << "powerTxW" << powerTxW << "\n";

  double txPowerDensity = (powerTxW / (15000*tone));
   int rbId;
  //NS_LOG_LOGIC ("Starting assign loop " );
  if(SUBCARRIER)
  {
    if(spacing==3)
    {
      txPowerDensity= (powerTxW / (3750*tone));
      //std::cout <<  " 3.75 Spacing" << "\n";
    }
  ...
  for (std::vector <int>::iterator it = activeRbs.begin (); it != activeRbs.end (); it++)
    {
       rbId = (*it);
        ...
        uint32_t start = rbId*12+sc_start;
        uint32_t end = rbId*12+sc_start+tone;
       // std::cout << "rbId " << rbId << "START and end " << start << " " << end <<"\n";
      for (uint32_t i = start; i< end ;i++)
       (*txPsd)[i] = txPowerDensity;
    }
}
else
{
  ...
  for (std::vector <int>::iterator it = activeRbs.begin (); it != activeRbs.end (); it++)
   ...
   //std::cout << "txPsd" << *txPsd << "\n";
/*if((*(txPsd))[0]==0)
                 std::cout << rbId << " txpsd " << *(txPsd)<< "\n";*/
