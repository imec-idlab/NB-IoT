    ...
    m_rsrpSinrSampleCounter (0)
{
  ...
  m_macChRxTtiDelay = DL_PDSCH_TTIS_DELAY;
  m_nooftti = 1;
  m_sfleft =0;
  m_ttileft=0;
  m_tone=12;
...
void
LteUePhy::SetSubChannelsForTransmission (std::vector <int> mask, bool srs)
{
  NS_LOG_FUNCTION (this);

  m_subChannelsForTransmission = mask;

  Ptr<SpectrumValue> txPsd = CreateTxPowerSpectralDensity1 (srs);
...
Ptr<SpectrumValue>
LteUePhy::CreateTxPowerSpectralDensity1 (bool srs)
{
  NS_LOG_FUNCTION (this);
  LteSpectrumValueHelper psdHelper;
  Ptr<SpectrumValue> psd;
  if (!srs)
  psd = psdHelper.CreateTxPowerSpectralDensity (m_ulEarfcn, m_ulBandwidth, m_txPower, GetSubChannelsForTransmission () , m_tone, m_scstart, m_spacing);
  else
   psd = psdHelper.CreateTxPowerSpectralDensity(m_ulEarfcn, m_ulBandwidth, m_txPower, GetSubChannelsForTransmission () , 12, 0, 15);
  return psd;
}
          ...
          if (msg)
            {
              ...
              //std::cout << "DoSendLteControlMessage m_p10CqiLast+m_p10CqiPeriodicity(msg)\n";
            }
      ...
      if  (Simulator::Now () > m_a30CqiLast + m_a30CqiPeriodicity)
        {
          ...
          if (msg)
            {
              DoSendLteControlMessage (msg);
              //std::cout << "DoSendLteControlMessage m_a30CqiLast + m_a30CqiPeriodicity(msg)\n";
            }
...
{
#ifdef LteUePhy_DEBUG
  NS_LOG_FUNCTION (this << " sinr:" << sinr);
#endif
          ...
          if (activeSubChannels > 0)
            {
              ...
              //std::cout << " ((uint16_t) cqiSum / activeSubChannels)" << ((uint16_t) cqiSum / activeSubChannels) << "\n";
            }
          else
            {
...
LteUePhy::ReceiveLteControlMessageList (std::list<Ptr<LteControlMessage> > msgList)
{
  ...
  //std::cout << " I am rnti = " << m_rnti << " and I received msgs " << (uint16_t) msgList.size () << "-----------------------\n";
      ...
      if (msg->GetMessageType () == LteControlMessage::DL_DCI)
        {
          ...
          std::cout << "LteControlMessage DL_DCI Reception...\n\n" ;
#endif 
                  ...
                  for (int k = 0; k < GetRbgSize (); k++)
                    {
                      ...
                      NS_LOG_DEBUG(this << " RNTI " << m_rnti << " RBG " << i << " [ue]DL-DCI allocated PRB " << (i*GetRbgSize()) + k);
#ifdef DCI_BW_DEBUG
                      std::cout << " RNTI " << m_rnti << " RBG " << i << " [ue]DL-DCI allocated PRB " << (i*GetRbgSize()) + k << "\n";
#endif
                    }
                }
              mask = (mask << 1);
          ...
          //=================================================================================
          int N_rb_dl = m_dlBandwidth;//m_dlBandwidth;
          int N_prb = dci.m_riv/N_rb_dl + 1;
          int RB_start = dci.m_riv % N_rb_dl;
          int repetitionNumber = dci.m_repetitionNumber;
#ifdef DCI_BW_DEBUG
          std::cout << " RNTI " << m_rnti << " m_dlBandwidth " << (int)m_dlBandwidth << " dci.m_riv:" << dci.m_riv << " dci.m_riv/N_rb_dl:" << dci.m_riv/N_rb_dl << " repetitionNumber:" << repetitionNumber ;
#endif          
          for (int j = 0; j < N_prb; j++)
          {
            dlRb.push_back (RB_start + j);
          }  
          QueueSubChannelsForReception (dlRb); 
#ifdef DCI_BW_DEBUG 
          std::cout << "Queue Sub-Channels For Reception...\n" ;
#endif     
          //dlRb.push_back (dci.m_riv % 100);
          NS_LOG_DEBUG(this << " RNTI " << m_rnti << " [ue]DL-DCI allocated PRB " << dci.m_riv % 100 << " N_prb:" << N_prb << " repetitionNumber: " << repetitionNumber);
#ifdef DCI_BW_DEBUG
          std::cout << " RNTI " << m_rnti << " [ue]DL-DCI allocated PRB " << dci.m_riv % 100 << " N_prb:" << N_prb << "\n";
#endif
          QueueDlDci (*msg2);
          ...
          //=============================================================================================================
          if(m_subChannelsForReceptionQueue.size()!=0 && m_dlDciQueue.size()!=0)
          {
            std::vector <int> dlRb = m_subChannelsForReceptionQueue.at (0);
            SetSubChannelsForReception (dlRb);
            // shift the queue
            for (uint8_t i = 1; i < m_macChRxTtiDelay; i++)
              {
                m_subChannelsForReceptionQueue.at (i-1) = m_subChannelsForReceptionQueue.at (i);
              }
            m_subChannelsForReceptionQueue.at (m_macChRxTtiDelay-1).clear ();
            //std::cout << "update info on TB to be received \n";
            // update info on TB to be received
            // reception of the control meesage is a good time to read the previous Dl-DCI
            std::list<DlDciLteControlMessage> dldcilist = DequeueDlDci ();
            std::list<DlDciLteControlMessage>::iterator dciIt = dldcilist.begin ();
            NS_LOG_DEBUG (this << " Ue Expected TBs " << dldcilist.size ());
            for (dciIt = dldcilist.begin (); dciIt!=dldcilist.end (); dciIt++)
              {
                // send info of TB to LteSpectrumPhy 
                NS_LOG_DEBUG (this << " UE " << m_rnti << " DL-DCI " << (*dciIt).GetDci ().m_rnti << " bitmap "  << (*dciIt).GetDci ().m_rbBitmap << " old DCI repetition: " << (*dciIt).GetDci ().m_repetitionNumber);
                //std::cout << this << " UE " << m_rnti << " DL-DCI " << (*dciIt).GetDci ().m_rnti << " bitmap "  << (*dciIt).GetDci ().m_rbBitmap << " old DCI repetition: " << (*dciIt).GetDci ().m_repetitionNumber;
                for (uint8_t i = 0; i < (*dciIt).GetDci ().m_tbsSize.size (); i++)
                {
                  m_downlinkSpectrumPhy->AddExpectedTb ((*dciIt).GetDci ().m_rnti, (*dciIt).GetDci ().m_ndi.at (i), (*dciIt).GetDci ().m_tbsSize.at (i), (*dciIt).GetDci ().m_mcs.at (i), dlRb, 18, 12, i, (*dciIt).GetDci ().m_harqProcess, (*dciIt).GetDci ().m_rv.at (i), true);
                }   
              }
          }
          //=============================================================================================================
        ...
        {
          //std::cout << "LteControlMessage UL_DCI Reception...\n\n" ;
          ...
          //=================================================================================
          int N_rb_ul = m_ulBandwidth;//m_dlBandwidth;
          int N_prb = dci.m_riv/N_rb_ul + 1;
          int RB_start = dci.m_riv % N_rb_ul;
          int repetitionNumber = dci.m_repetitionNumber;
          for (int j = 0; j < N_prb; j++)
          {
            ulRb.push_back (RB_start + j);
          }  
          /*std::cout << "1A Hi\n";
          for(int j = 0 ; j < (int)ulRb.size(); j++)//sizeof (rbMask) /sizeof (rbMask[0])
            std::cout << ulRb[j] << ' ';
          std::cout << "\n"; */
       //  m_subChannelsForTransmissionQueue.clear ();
         //  std::vector <int> ul;
     //  m_subChannelsForTransmissionQueue.resize (m_macChTtiDelay, ul);
          ...
          std::cout << (*printer) << " (QueueSubChannelsForTransmission) RNTI " << m_rnti << " [ue]UL-DCI allocated PRB " << dci.m_riv % 100 << " N_prb:" << N_prb << " repetitionNumber: " << repetitionNumber << " RB_start:" << RB_start << "\n";     
          */
          NS_LOG_DEBUG(this << " RNTI " << m_rnti << " [ue]UL-DCI allocated PRB " << dci.m_riv % 100 << " N_prb:" << N_prb << " repetitionNumber: " << repetitionNumber);
          ...
          m_timestamp = params.m_timestamp;
          std::cout << " TIME " << Simulator::Now().GetSeconds() << " RNTI " << m_rnti << " [ue]UL-DCI allocated PRB " << dci.m_riv % 100 << " N_prb:" << N_prb << " m_timestamp: " << m_timestamp << "\n";         
          ...
          // USE THE SUBCARRIER INDICATION FIELD OF DCI TO FRAME THE NO OF TTI'S OVER WHICH THE TB NEEDS TO BE EXTENDED BASED ON THE TONE:---------------------------------------------------------------------------------------
         std::cout << "sc indic " << (int)(dci.m_scindication) << "\n";
         if (dci.m_scindication >=19 )
          {
            m_scstart = dci.m_scindication-19;
            m_nooftti = 32;
            m_tone =1;
            m_spacing=3;

           // std::cout << "single tone with sc start " << m_scstart << " #tti "<<  m_nooftti <<  " RNTI " << m_rnti <<  "\n";
          }        
          else if (dci.m_scindication <= 11 && dci.m_scindication >=0 )
          {
            m_scstart = dci.m_scindication;
            m_nooftti = 8;
            m_tone =1;
            m_spacing=15;
          ...
          else if (dci.m_scindication <= 15 && dci.m_scindication >11 )
          {
            m_scstart = (dci.m_scindication-12)*3;
            m_nooftti = 4;
            m_tone = 3;
            m_spacing=15;
           // std::cout << "3 tone with sc start " << m_scstart << " #tti "<<  m_nooftti << " RNTI " << m_rnti << "\n";
          }
          else if (dci.m_scindication <= 17 && dci.m_scindication >15 )
          {
            m_scstart = (dci.m_scindication-16)*6;
            m_nooftti = 2;
            m_tone=6;
            m_spacing=15;
          //  std::cout << "6 tone with sc start " << m_scstart << " #tti "<<  m_nooftti << " RNTI " << m_rnti <<"\n\n\n\n";
          }
          else if (dci.m_scindication == 18 )
          {
            m_scstart = 0;
            m_nooftti = 1;
            m_tone=12;
            m_spacing=15;
            //std::cout << "12 tone with sc start " << m_scstart << " #tti "<<  m_nooftti << " RNTI " << m_rnti <<  " rep " << dci.m_repetitionNumber << "\n\n\n\n";
          }

          m_repetitionNumber = dci.m_repetitionNumber;
              ...
              for (std::list<RarLteControlMessage::Rar>::const_iterator it = rarMsg->RarListBegin (); it != rarMsg->RarListEnd (); ++it)
                {
                  if (it->rapId != m_raPreambleId)
                  ...
                  else
                    {
                      ...
                      int N_prb = it->rarPayload.m_grant.m_riv/N_rb_ul + 1;
                      int RB_start = it->rarPayload.m_grant.m_riv % N_rb_ul;
                      for (int j = 0; j < N_prb; j++)
                      {
                        ulRb.push_back (RB_start + j);
                      }
#ifdef DCI_BW_DEBUG
                      int repetitionNumber = it->rarPayload.m_grant.m_repetitionNumber;
                      std::cout << " RNTI " << m_rnti << " m_ulBandwidth " << (int)m_ulBandwidth << " N_prb:" << N_prb << " RB_start:" << RB_start << " rarPayload.m_grant.m_riv:" << it->rarPayload.m_grant.m_riv << " rarPayload.m_grant.m_riv/N_rb_ul:" << it->rarPayload.m_grant.m_riv/N_rb_ul << "   " << repetitionNumber << "\n";
#endif
                      QueueSubChannelsForTransmission (ulRb);
                      // pass the info to the MAC
                      m_uePhySapUser->ReceiveLteControlMessage (msg);
...
void
LteUePhy::QueueSubChannelsForReception (std::vector <int> rbMap)
{
  NS_LOG_FUNCTION (this << (int)m_macChRxTtiDelay);
#ifdef DEBUG_CONTROL_DATA_QUEUE
  std::cout << "m_ulConfigured ===> m_subChannelsForReceptionQueue.size():" << m_subChannelsForReceptionQueue.size() << " m_subChannelsForReceptionQueue.at (m_macChRxTtiDelay - 1) " << m_subChannelsForReceptionQueue.at (m_macChRxTtiDelay - 1).size() << "\n";
  std::cout << "m_macChRxTtiDelay:" << (int)m_macChRxTtiDelay << "\n";
#endif
  m_subChannelsForReceptionQueue.at (m_macChRxTtiDelay - 1) = rbMap;
#ifdef DEBUG_CONTROL_DATA_QUEUE
  std::cout << "m_ulConfigured ===> m_subChannelsForReceptionQueue.size():" << m_subChannelsForReceptionQueue.size() << " m_subChannelsForReceptionQueue.at (m_macChRxTtiDelay - 1) " << m_subChannelsForReceptionQueue.at (m_macChRxTtiDelay - 1).size() << "\n";
#endif  
}
...
//std::cout <<  " UE - subframe " << " frame " << frameNo << " subframe " << subframeNo << " srs period "<< m_srsPeriodicity << "\n" ;
  ...
  if (m_ulConfigured)
    {
#ifdef DEBUG_CONTROL_DATA_QUEUE
      std::cout << "m_ulConfigured ===> m_subChannelsForReceptionQueue.size():" << m_subChannelsForReceptionQueue.size() << "\n";
#endif
      //============================================================================================
      // update dllink reception mask according to previous DL-CQIs
      //std::vector <int> rbMask = m_subChannelsForReceptionQueue.at (0);
/*      if(m_subChannelsForReceptionQueue.size()!=0 && m_dlDciQueue.size()!=0)
      {
        std::vector <int> dlRb = m_subChannelsForReceptionQueue.at (0);
        SetSubChannelsForReception (dlRb);
        ...
        for (dciIt = dldcilist.begin (); dciIt!=dldcilist.end (); dciIt++)
          {
            // send info of TB to LteSpectrumPhy 
            NS_LOG_DEBUG (this << " UE " << m_rnti << " DL-DCI " << (*dciIt).GetDci ().m_rnti << " bitmap "  << (*dciIt).GetDci ().m_rbBitmap);
            for (uint8_t i = 0; i < (*dciIt).GetDci ().m_tbsSize.size (); i++)
            {
              m_downlinkSpectrumPhy->AddExpectedTb ((*dciIt).GetDci ().m_rnti, (*dciIt).GetDci ().m_ndi.at (i), (*dciIt).GetDci ().m_tbsSize.at (i), (*dciIt).GetDci ().m_mcs.at (i), dlRb, i, (*dciIt).GetDci ().m_harqProcess, (*dciIt).GetDci ().m_rv.at (i), true);
            }   
          }
      }*/
      //============================================================================================
#ifdef DEBUG_CONTROL_DATA_QUEUE
      std::cout << "[UE] update uplink transmission mask according to previous UL-CQIs m_rnti:" << m_rnti <<  " \n";
#endif
      // update uplink transmission mask according to previous UL-CQIs
      std::vector <int> rbMask;
       //if(!m_subChannelsForTransmissionQueue.empty())
      {rbMask = m_subChannelsForTransmissionQueue.at (0);
    ...
    //if(!m_subChannelsForTransmissionQueue.empty())
    {
      ...
      if (m_srsConfigured && (m_srsStartTime <= Simulator::Now ()))
        {
          ...
          if (( Now().GetMilliSeconds() % (m_srsPeriodicity) )== m_srsSubframeOffset)
            {
              NS_LOG_INFO ("frame " << frameNo << " subframe " << subframeNo << " sending SRS (offset=" << m_srsSubframeOffset << ", period=" << m_srsPeriodicity << ")");
              //std::cout << "frame " << frameNo << " subframe " << subframeNo << " sending SRS (offset=" << m_srsSubframeOffset << ", period=" << m_srsPeriodicity << ")" << "\n";
      ...
      std::list<Ptr<LteControlMessage> > ctrlMsg = GetControlMessages (subframeNo);
      ...
      if(RETRANSMIT && m_nRetransmissions > 0)
      {
         if(m_ttileft==0)
          {if (m_enableUplinkPowerControl)
            { 
              
              m_txPower = m_powerControl->GetPuschTxPower (m_prevrb);
              SetSubChannelsForTransmission (m_prevrb);
              /*for(std::vector<int>::const_iterator j = rbMask.begin(); j != rbMask.end(); ++j)
                std::cout << *j << ' ';
              std::cout << "\n";*/
            }
        //std::cout << " UE  -->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->--> " << m_rnti << " Repetition " << m_nRetransmissions << " time " <<  Simulator::Now().GetSeconds() << " RNTI " << m_rnti <<  "\n\n";
        m_uplinkSpectrumPhy->StartTxDataFrame (m_RetransmitPacket, m_prevctrlMsg, UL_DATA_DURATION, m_scstart,m_tone );
 ...
 if (pb )
        {
          ...
          NS_LOG_LOGIC (this << " UE - StartTxDataFrame " << " -->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->");
          /*ns3::LogTimePrinter printer = ns3::LogGetTimePrinter (); 
          if (printer != 0)                                         
            {                                                       
              (*printer)(std::clog);                                
            }
          std::cout << (*printer) << " UE - StartTxDataFrame " << " -->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->--> m_rnti:" << m_rnti << "\n";
          */
   
        //MODIFY BASED ON UL_DCI, BASED ON TONE, m_nooftti will change--------------------------------------------------------------------------------------------------------------------------------------
           //std::cout << " UE " << m_rnti << " start tx data, time " <<  Simulator::Now().GetSeconds() <<  " dura " << m_nooftti << "\n";
          if(!rbMask.empty())
         // std::cout << " UE  -->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->--> " << m_rnti << " RB "<< rbMask.at(0) << " start tx data, time " <<  Simulator::Now().GetSeconds()  << " TONE "  << m_tone << " m_txPower " << m_txPower << " for " << UL_DATA_DURATION * m_nooftti << " \n\n\n\n";
          m_prevrb = rbMask;
          m_RetransmitPacket = pb; 
          m_prevctrlMsg = ctrlMsg;
           m_sfleft = (m_repetitionNumber*m_nooftti);
           m_ttileft = m_nooftti-1;
          if(m_repetitionNumber > 1)
          {
            m_nRetransmissions = m_repetitionNumber - 1;
            m_repetitionNumber=1;
          }
          if(EXTENDED ) 
          {  //if(m_sfleft==0)
               m_uplinkSpectrumPhy->StartTxDataFrame (pb, ctrlMsg, UL_DATA_DURATION * m_nooftti,m_scstart,m_tone);
             //else 
              // m_sfleft--;
                      }
        else 
          ...
          if (ctrlMsg.size ()>0)
            {
             ...
             std::vector <int> dlRb;
              ...
              int rb_len =1;
               ...
               for (uint8_t i = 0; i < rb_len; i++) {  
               dlRb.push_back (rb_start + i);
               }*/
              ...
              NS_LOG_LOGIC (this << " UE - StartTxDataFrame TX PUCCH (NO PUSCH) " << " -->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->-->");
              SetSubChannelsForTransmission (dlRb);
              //std::cout << " UE  no PB" << m_nRetransmissions << " start tx data, RB, TIME " << " " <<Simulator::Now().GetSeconds() << "\n";
              if(m_sfleft==0)
               m_uplinkSpectrumPhy->StartTxDataFrame (pb, ctrlMsg, UL_DATA_DURATION,m_scstart,m_tone);
             else 
               m_sfleft--;
            }
          else
  ...
  //MODIFY BASED ON UL_DCI, BASED ON TONE, m_nooftti will change--------------------------------------------------------------------------------------------------------------------------------------------------------------------
  if(EXTENDED) 
    subframeNo = subframeNo + 1;//m_nooftti;
  else
...
LteUePhy::SendSrs ()
{
  NS_LOG_FUNCTION (this << " UE " << m_rnti << " start tx SRS, cell Id " << (uint32_t) m_cellId);
   //std::cout << " UE " << m_rnti << " start tx SRS, time "  << Simulator::Now().GetSeconds() << " offset " << m_srsSubframeOffset << "\n\n\n";
  std::vector <bool> ulRbMap;
  // set the current tx power spectral density (full bandwidth)
  std::vector <int> dlRb;
  int rb_len;
  int rb_start = 0;
   std::vector <int> BW15  =  {2, 12};  ///*
    int rb_starting [][4]= {
  { 1, 6, 0, 2},
  { 2, 6, 2, 2},
  { 3, 6, 4, 2},
...
{
  if(m_ulBandwidth == rb_starting[i][1])
  {
    if((m_cellId % 3) == 1)
  {
    rb_start = 0;
    rb_len = rb_starting[i][3];
  }
  else if((m_cellId % 3) ==  2)
  {
    rb_start = rb_starting[i+1][2]; //32
     rb_len = rb_starting[i+1][3];
  }
  else if((m_cellId % 3) ==  0)
  {
    rb_start =rb_starting[i+2][2]; //64
    rb_len = rb_starting[i+2][3];
...
rb_start = m_prevrb.at(0);
rb_start=23;
rb_len = 1;//m_ulBandwidth;// m_ulBandwidth;
for (uint8_t i = 0; i < rb_len; i++)
  {
  ...
  /*std::cout << "dlRb:";
  for (std::vector<int>::const_iterator i = dlRb.begin(); i != dlRb.end(); ++i)
    std::cout << *i << ' ';
  std::cout << "\n";
  std::cout << " m_txPower:" << m_txPower << "\n";*/
 //std::cout << " UE " << m_rnti << " start tx SRS, time " << " RB "<< rb_start << " RBLEN " << rb_len << " " << Simulator::Now().GetSeconds() << " offset " << m_srsSubframeOffset << "\n\n\n";
  SetSubChannelsForTransmission (dlRb, true);
  m_uplinkSpectrumPhy->StartTxUlSrsFrame (m_rnti);
}
...
LteUePhy::DoReset ()
{
  ...
  m_subChannelsForTransmissionQueue.clear ();
  m_subChannelsForReceptionQueue.clear();
  ...
  std::vector <int> dlRb;
  m_subChannelsForReceptionQueue.resize(m_macChRxTtiDelay, dlRb);
...
LteUePhy::DoSetDlBandwidth (uint8_t dlBandwidth)
{
  ...
  m_dlConfigured = true;
  //std::cout << "m_rbgSize:"  << (int)m_rbgSize << " dlBandwidth:" << (int)dlBandwidth << "\n";
}
...
m_mac->setsrstime(m_srsSubframeOffset, m_rnti);
  ...
  //std::cout << " UE SRS P " << m_srsPeriodicity << " RNTI " << m_rnti << " offset " << m_srsSubframeOffset << " cellId " << m_cellId << " CI " << srcCi << "\n\n\n\n";
}
...
LteUePhy::DequeueDlDci (void)
{
  NS_LOG_FUNCTION (this);
#ifdef DEBUG_CONTROL_DATA_QUEUE
  std::cout << "LteUePhy::QueueDlDci:  m_dlDciQueue.size(): " << m_dlDciQueue.size() << " m_dlDciQueue.at (0).size(): " << m_dlDciQueue.at (0).size() << "\n";
#endif  
...
void
LteUePhy::QueueDlDci (DlDciLteControlMessage m)
{
  NS_LOG_FUNCTION (this);
#ifdef DEBUG_CONTROL_DATA_QUEUE
  std::cout << "LteUePhy::QueueDlDci:  m_dlDciQueue.size(): " << m_dlDciQueue.size() << " m_dlDciQueue.at (0).size(): " << m_dlDciQueue.at (0).size() << "\n";
