...
public:
  ...
  */
  bool StartTxDataFrame (Ptr<PacketBurst> pb, std::list<Ptr<LteControlMessage> > ctrlMsgList, Time duration,uint32_t scstart= 0, uint32_t tone = 12);
  ...
  * started, false otherwise.
  */
  bool StartTxUlSrsFrame (uint32_t rnti);
  ...
  */
  void AddExpectedTb (uint16_t  rnti, uint8_t ndi, uint16_t size, uint8_t mcs, std::vector<int> map, uint32_t scindic,uint32_t tone, uint8_t layer, uint8_t harqId, uint8_t rv, bool downlink, uint32_t m_repetitionNumber=1);
...
private:
  ...
  void EndTxDlCtrl ();
  void EndTxUlSrs ();
  void EndRxData (Ptr<LteSpectrumSignalParametersDataFrame> params);
  void EndRxDlCtrl ();
  void EndRxUlSrs ();
  ...
  SpectrumValue m_sinrPrevious;
  SpectrumValue m_sinrsum;
