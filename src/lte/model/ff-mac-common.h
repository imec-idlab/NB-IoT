...
{
  bool      m_flag; //0-> N0 format, 1-> N1 format
  uint8_t   m_scindication; //Subcarrier indication between 0 to 64 6 bits;
  uint8_t   m_numberofRU; //No of Resource units 0 to 7
  uint8_t   m_scheddelay; //Scheduling delay a number between 0 to 3
  uint8_t   m_mcs; //MCS refer table
  bool      m_rv; //Redundancy version 1 or 0
  uint32_t  m_repetitionNumber;//REpetition number new for NB-IOT
  uint8_t   m_ndi; //New data indicator 1 or 0
  uint8_t   m_dcirepetitionNumber; //2 bits


//All these are not mentioned in NB IOT spec and are from LTE:
  uint16_t  m_rnti;
  uint8_t   m_rbStart;
  uint8_t   m_rbLen;
  uint32_t  m_rbBitmap;
