...
UeManager::DoInitialize ()
{
  ...
  m_rrcreleaseinterval  = m_rrc->m_rrcreleaseinterval_d;
  m_enablePSM           = m_rrc->m_enablePSM_d;
  ...
  switch (m_state)
    ...
    case INITIAL_RANDOM_ACCESS:
      ...
      // Add RRC release feature

      std::cout<<Simulator::Now().GetSeconds()<<" "<<m_rnti<<" PSM Enable: "<< m_enablePSM<<" m_rrcreleaseinterval: "<<m_rrcreleaseinterval<<std::endl;
...
UeManager::SendData (uint8_t bid, Ptr<Packet> p)
{
  ...
  std::cout<<Simulator::Now().GetSeconds()<<" "<<GetRnti()<<" LteEnbRrc::SendData "<<m_state<<" "<<p->GetSize()<<" UID: "<<p->GetUid()<<std::endl;
  switch (m_state)
    {
    case INITIAL_RANDOM_ACCESS:
    case CONNECTION_SETUP:
    ...
    case CONNECTED_NORMALLY:
        {
            ...
            {
                EventId id;
               // In case data is received, remove all the RRC release request and generate a new one
    ...
    case CONNECTION_SUSPEND:
        /* Save data */
        /* send paging */
        /* then send the saved data (also that is saved during PSM): This will happen during the CONNECTED_NORMALLY state */
    case CONNECTION_PSM:
        /* save data and do paging in Suspend mode or wait for connected mode */
        break;
...
UeManager::DoReceivePdcpSdu (LtePdcpSapUser::ReceivePdcpSduParameters params)
{
  ...
  if (CONNECTION_SUSPEND == m_state || CONNECTION_PSM == m_state)
  {
      m_datareceived = true;
      SwitchToState (CONNECTED_NORMALLY);
      m_rrc->SendData(NULL);
...
UeManager::SwitchToState (State newState)
{
  ...
  /* Case of cancelling PSM while current is Normal and when data is being received cancel PSM and Suspend state change */
  if ( (true == m_datareceived && (CONNECTION_SUSPEND == newState || CONNECTION_PSM == newState)) || (CONNECTION_PSM == newState && CONNECTED_NORMALLY == m_state ) )
  {
      return;
  }
  ...
  switch (newState)
    ...
    case CONNECTION_SUSPEND:
        ...
        {
            NS_LOG_FUNCTION (this << ToString (newState) << Simulator::Now() << m_t3324 << "Scheduled PSM for:"<<Simulator::Now() +  (m_t3324));
            id = Simulator::Schedule ( (m_t3324), &UeManager::SwitchToState, this, CONNECTION_PSM);
    ...
    case CONNECTION_PSM:
        /* complete */
        NS_LOG_FUNCTION (this << ToString (newState) << Simulator::Now() <<  m_t3412 - m_t3324 << "Scheduled Suspend for:"<<Simulator::Now() + m_t3412 - m_t3324);
        id = Simulator::Schedule ( m_t3412 - m_t3324, &UeManager::SwitchToState, this, CONNECTION_SUSPEND);
        id_suspend.push_back(id);
...
LteEnbRrc::GetTypeId (void)
{
  ...
  static TypeId tid = TypeId ("ns3::LteEnbRrc")
    ...
    .AddAttribute ("RrcReleaseInterval",
              ...
              UintegerValue(10000),
              MakeUintegerAccessor (&LteEnbRrc::m_rrcreleaseinterval_d),
              MakeUintegerChecker<uint16_t> (0, 60000) )
    .AddAttribute ("EnablePSM",
               "If true, PSM will be enabled.",
               BooleanValue (true),
               MakeBooleanAccessor (&LteEnbRrc::m_enablePSM_d),
               MakeBooleanChecker ())
  ...
  /*if(m_nbIotActiveMode)
    {
      ...
      */ /*
       ...
       */
      //Simulator::Schedule (MilliSeconds (16), &NbLteRrcSap::SendSystemInformationNb, this);
    /*}
  else
    { */
...
LteEnbRrc::SendData (Ptr<Packet> orgpacket)
{
  ...
  if (orgpacket == NULL)
  {
      ...
      if (m_packetsaved.size() == 0)//if (m_packetsaved.at(0) == 0 && m_packetsaved.size() == 1)
      {
          return true;
      }
      // Send all the saved packets during PSM mode.
  ...
  {
      Ptr<Packet> packet = orgpacket->Copy(); // copy the original packet
      ...
      // TODO: Need to evaluate if we need it here or not
      /*for (auto &it: m_componentCarrierPhyConf)
      {
        uint8_t ccId = it.first;
        m_cphySapProvider.at (ccId)->UpdatePagingParam();
      }*/
      ...
      /*for (uint8_t i = 0; i < m_numberOfComponentCarriers; i++)
        {
          ...
          m_cphySapProvider.at (i)->UpdatePagingParam();
        }*/


      NS_LOG_FUNCTION (this << ueManager->m_state);
      if ( ueManager->CONNECTION_SUSPEND == ueManager->m_state || ueManager->CONNECTION_PSM == ueManager->m_state)
      {
              /*  Save the packet */
          //std::vector< Ptr<Packet> >::iterator it = m_packetsaved.begin();
          /*if (m_packetsaved.at(0) == 0)
          {
                  NS_LOG_FUNCTION (this << "if");
                  m_packetsaved.at(0) = orgpacket;
          }
          else*/
          {
...
void UeManager::SwitchtoDRXPSMState()
{
    NS_LOG_FUNCTION (this);
    //Simulator::ScheduleNow(&UeManager::SwitchToState, this, CONNECTION_SUSPEND);
    SwitchToState(CONNECTION_SUSPEND);
}

void UeManager::SendRrcRelease(uint16_t rnti)
{
    NS_LOG_FUNCTION (this );
    std::cout<<Simulator::Now().GetSeconds()<<" "<<rnti<< " SendRrcRelease "<< g_ueManagerStateName[m_state]<<" m_lastRrcTransactionIdentifier: "<<m_lastRrcTransactionIdentifier<<std::endl;
    EventId id;
    if (CONNECTED_NORMALLY == m_state)
    {
...
static const uint8_t SRS_ENTRIES = 10;
/**
...
static const uint32_t g_srsPeriodicity[SRS_ENTRIES] = {0, 2, 5, 10, 20, 40, 80, 160, 320, 640};//, 1280, 2560, 5120, 10240, 20480, 40960, 81920};
/**
...
static const uint32_t g_srsCiLow[SRS_ENTRIES] =       {0, 0, 2, 7, 17, 37, 77, 157, 317, 637};//, 1277, 2557, 5117, 10237, 20477, 40957, 81917};
/**
...
static const uint32_t g_srsCiHigh[SRS_ENTRIES] =      {0, 1, 6, 16, 36, 76, 156, 316, 636, 1276};//, 2556, 5116, 10236, 20476, 40956, 81916, 163836};
