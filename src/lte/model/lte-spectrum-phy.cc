    ...
    m_layersNum (1)
{
  ...
  m_sinrPrevious=0;
  m_sinrsum =0;
  m_random->SetAttribute ("Min", DoubleValue (0.1));
  m_random->SetAttribute ("Max", DoubleValue (0.1));
...
bool
LteSpectrumPhy::StartTxDataFrame (Ptr<PacketBurst> pb, std::list<Ptr<LteControlMessage> > ctrlMsgList, Time duration, uint32_t scstart, uint32_t tone)
{
  NS_LOG_FUNCTION (this << pb);
  NS_LOG_LOGIC (this << " state: " << m_state);
  
  m_phyTxStartTrace (pb);
 // std::cout << "StartTxDataFrame " << m_state << "\n";
  ...
  switch (m_state)
    ...
    case RX_UL_SRS:
      ...
      break;
    case TX_DATA:  std::cout << "cannot TX while already TX data: the MAC should avoid this\n"; break;
    case TX_DL_CTRL: std::cout << "cannot TX while already TX CTL: the MAC should avoid this\n"; break;
    case TX_UL_SRS: std::cout << "cannot TX while already TX SRS: the MAC should avoid this\n"; 
     break;
    ...
    case IDLE:
    {
     ...
     // std::cout <<"POSITION " << txMobility->GetPosition () <<"\n";
       
      txParams->txAntenna = m_antenna;
      txParams->psd = m_txPsd;
      //txParams->txMobility = txMobility;
      //std::cout << "StartTxDataFrame " << duration << "\n";
...
bool
LteSpectrumPhy::StartTxUlSrsFrame (uint32_t rnti)
{
  NS_LOG_FUNCTION (this);
  NS_LOG_LOGIC (this << " state: " << m_state);
  ...
  switch (m_state)
    ...
    case RX_DATA:
    case RX_DL_CTRL:
    case RX_UL_SRS:std::cout << "cannot TX SRS while already Rx SRS: the MAC should avoid this\n"; 
      //NS_FATAL_ERROR ("cannot TX while RX: according to FDD channel acces, the physical layer for transmission cannot be used for reception");

      break;
      
    case TX_DL_CTRL:  std::cout << "cannot TX SRS while already TX ctrl: the MAC should avoid this\n";   //NS_FATAL_ERROR ("cannot TX while already TX ctrl: the MAC should avoid this");
      break;
   
    case TX_UL_SRS: std::cout << "cannot TX SRS while already TX srs: the MAC should avoid this\n"; 
      NS_FATAL_ERROR ("cannot TX SRS while already TX SRS: the MAC should avoid this");
      break;
    case TX_DATA:  std::cout << "cannot TX SRS while already TX data: the MAC should avoid this\n"; //NS_FATAL_ERROR ("cannot TX while already TX data: the MAC should avoid this");
    ...
    case IDLE:
    {
      ...
      */
      ...
      txParams->txPhy = GetObject<SpectrumPhy> ();
      Ptr<MobilityModel> txMobility = txParams->txPhy->GetMobility ();
      Vector pos = txMobility->GetPosition ();
      std::cout << "rnti " << rnti << " POSITION " << pos.x << " " << pos.y<< " " << pos.z <<"\n";
      txParams->txAntenna = m_antenna;
      txParams->psd = m_txPsd;
      txParams->cellId = m_cellId;
      m_channel->StartTx (txParams);
...
{
  NS_LOG_FUNCTION (this);
  NS_LOG_LOGIC (this << " state: " << m_state << "--f--f--f--f--f--f--f--f--f--f--f--f--f--f--f--f--f--f--f--f--f--f--f--f--f--f--f--f--f");
...
LteSpectrumPhy::StartRx (Ptr<SpectrumSignalParameters> spectrumRxParams)
{
  ...
  //std::cout << Now().GetSeconds()<< " START RX "<< spectrumRxParams->duration<< "\n";
  ...
  switch (m_state)
    {
      ...
      case RX_DATA:
        ...
        // simultaneously (e.g., at the eNB).
        {
          ...
          //  LteSpectrumSignalParametersDataFrame
                 //if(params->duration > NanoSeconds (928570))
                 // std::cout << "Rxed duration " << params->duration << "\n"; 
          if (params->cellId  == m_cellId)
            {
              ...
              if ((m_rxPacketBurstList.empty ())&&(m_rxControlMessageList.empty ()))
                ...
                if(params->duration > NanoSeconds (928570))
                  ...
                  m_endRxDataEvent = Simulator::Schedule (params->duration, &LteSpectrumPhy::EndRxData, this, params);
                }
              else
                {
                  ...
                  // won't be correct
                 // std ::cout << Simulator::Now () << " m_firstRxStart " << m_firstRxStart << " dur:1st rx" << m_firstRxDuration << " dur " << params->duration << "\n";
            ...
            {
                if(params->duration > NanoSeconds (928570))
                  //std::cout << "Rxed cell id mismatch " << params->duration << "\n"; 
...
LteSpectrumPhy::StartRxUlSrs (Ptr<LteSpectrumSignalParametersUlSrsFrame> lteUlSrsRxParams)
{
  ...
  switch (m_state)
    ...
    case RX_DATA://NS_FATAL_ERROR ("cannot RX SRS while receiving data else");
      //break;
    case RX_DL_CTRL:
    case TX_DATA: 
   ...
   // break;
    ...
    case RX_UL_SRS:
     ...
     // std::cout << " PSD " << *(lteUlSrsRxParams->psd) <<"\n";
        uint16_t cellId;
...
void
LteSpectrumPhy::AddExpectedTb (uint16_t  rnti, uint8_t ndi, uint16_t size, uint8_t mcs, std::vector<int> map, uint32_t scindic, uint32_t tone, uint8_t layer, uint8_t harqId,uint8_t rv,  bool downlink , uint32_t m_repetitionNumber)
{
  NS_LOG_FUNCTION ( " rnti: " << rnti << " NDI " << (uint16_t)ndi << " size " << size << " mcs " << (uint16_t)mcs << " layer " << (uint16_t)layer << " rv " << (uint16_t)rv);
  uint32_t scstart =0;
  TbId_t tbId;
  tbId.m_rnti = rnti;
  tbId.m_layer = layer;
  ...
  if (it != m_expectedTbs.end ())
    ...
    if (scindic <= 11 && scindic >=0 )
          {
            scstart = scindic;
            tone =1;
         
          }
          else if (scindic <= 15 && scindic >11 )
          {
          scstart = (scindic-12)*3;
        tone = 3;
        
          }
          else if (scindic <= 17 && scindic >15 )
          {
          scstart = (scindic-16)*6;
          tone=6;
 ...
 //m_repetitionleft = m_repetitionNumber-1;
  // insert new entry
  tbInfo_t tbInfo = {ndi, size, mcs, map,scstart,tone, harqId, rv, 0.0, downlink, false, false, m_repetitionNumber};
  //for(int i=0;i<m_repetitionNumber;i++)
    m_expectedTbs.insert (std::pair<TbId_t, tbInfo_t> (tbId,tbInfo));
}


void
LteSpectrumPhy::EndRxData (Ptr<LteSpectrumSignalParametersDataFrame> params)
{
  NS_LOG_FUNCTION (this);
  NS_LOG_LOGIC (this << " state: " << m_state);
  Ptr<SpectrumValue> SINR100 = Create <SpectrumValue> (m_rxSpectrumModel);
Values::const_iterator it;
  ...
  m_sinrsum = m_sinrPerceived; 
  int count=0;
  while (itTb!=m_expectedTbs.end ())
  {
    Values::iterator it1 = m_sinrsum.ValuesBegin ();
    Values::const_iterator it2 = m_sinrPrevious.ValuesBegin ();
    int EnableBLER = 1;

    if((*itTb).second.repetition  > 1 )
    {
//std::cout << " SINR RECEIVED " << m_sinrPerceived << "\n";
 //std::cout << " SINR SUM " << m_sinrsum << "\n";
 //std::cout << " SINR PREV" << m_sinrPrevious << "\n";
      while (it1 != m_sinrsum.ValuesEnd () && it2 != m_sinrPrevious.ValuesEnd ())
      {
        if( it2 != m_sinrPrevious.ValuesEnd ())
        {
          *it1 += *it2;
          ++it1;
          ++it2;
    ...
    NS_LOG_DEBUG (this << " m_dataErrorModelEnabled " << (int)m_dataErrorModelEnabled << " m_rxPacketBurstList.size () " << m_rxPacketBurstList.size ());
      if ((m_dataErrorModelEnabled)&&(m_rxPacketBurstList.size ()>0)) // avoid to check for errors when there is no actual data transmitted
      {
        ...
        {
          m_sinrPrevious[(*itTb).second.rbBitmap.at(0)]=0;
        ...
        if ((*itTb).second.ndi == 0 || (*itTb).second.repetition == 1)
        {
         EnableBLER = 1;
        }
        else
       ...
       if(EnableBLER)
       {
        int rbno = (*itTb).second.rbBitmap.at(0);
        if ((*itTb).second.ndi == 0)
          {m_sinrPrevious[rbno]=0;
         // std::cout << rbno << " here SINR 0 due to "<<   (*itTb).first.m_rnti << "\n";
          }

        uint32_t i=0;
        uint32_t start = rbno*12+(*itTb).second.scstart;
        uint32_t end = rbno*12+(*itTb).second.scstart+(*itTb).second.tone;
        double sum =0.0;
        for (it = m_sinrsum.ConstValuesBegin (); it != m_sinrsum.ConstValuesEnd (); it++)
        {
      ...
      (*SINR100)[rbno] = sum/(*itTb).second.tone;
       TbStats_t tbStats;
       HarqProcessInfoList_t harqInfoList1; 

      if(SUBCARRIER)
      {
            //std::cout << Now().GetSeconds() <<"  SINR100 " << *SINR100 << "\n";
       tbStats = LteMiErrorModel::GetTbDecodificationStats (*SINR100, (*itTb).second.rbBitmap, (*itTb).second.size, (*itTb).second.mcs, harqInfoList1);
     }
     else
        {//std::cout << rbno << "  " << m_sinrPerceived << "\n";
          tbStats = LteMiErrorModel::GetTbDecodificationStats (m_sinrsum, (*itTb).second.rbBitmap, (*itTb).second.size, (*itTb).second.mcs, harqInfoList1);
        }
        (*itTb).second.mi = tbStats.mi;
        
        (*itTb).second.corrupt = m_random->GetValue () > tbStats.tbler ? false : true;

       //std::cout << Now().GetSeconds() << " RNTI " << (*itTb).first.m_rnti <<  " MCS " << int((*itTb).second.mcs) << " SIZE " << (*itTb).second.size << " CORRUPT BIT " << (*itTb).second.corrupt << " BLER " << tbStats.tbler << "  END RX " << params->duration << " SC START " << (*itTb).second.scstart << " TONE " << (*itTb).second.tone << " NDI "<< int((*itTb).second.ndi) << " RB " << (*itTb).second.rbBitmap.at(0)<<  " SINR " << *SINR100 << "\n";
       
       }
        else
           {(*itTb).second.corrupt = true;
            //std::cout << Now().GetSeconds() << " ELSE CORRUPT BIT " << m_sinrsum << "\n";
     } 
          
        //Ptr<MobilityModel> txMobility = params->txPhy->GetMobility ();
       // std::cout<< "RNTI "<< (*itTb).first.m_rnti << " AT " << txMobility->GetPosition() << "\n";;
     ...
     //std::cout << " START OF LOOP 2 " <<  m_rxPacketBurstList.size () << "\n";
  ...
  if (m_ctrlErrorModelEnabled)
    {
      ...
      {
        //std::cout << " PCFICH-PDCCH Decodification, errorRate " << errorRate << " error " << error << "\n";
        //NS_ASSERT(0);
      }
    }
