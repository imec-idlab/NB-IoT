...
MacStatsCalculator::UlScheduling (uint16_t cellId, uint64_t imsi, uint32_t frameNo,
                                  uint32_t subframeNo, uint16_t rnti,uint8_t mcsTb, uint16_t size, uint8_t componentCarrierId, uint32_t repetition)
{
  ...
  if ( m_ulFirstWrite == true )
    {
      ...
      outFile << "% time\tcellId\tIMSI\tframe\tsframe\tRNTI\tmcs\tsize\trepetition\tccId";
      outFile << std::endl;
    }
  else
    {
...
MacStatsCalculator::UlSchedulingCallback (Ptr<MacStatsCalculator> macStats, std::string path,
                      ...
                      uint8_t mcs, uint16_t size, uint8_t componentCarrierId, uint32_t repetition)
{
  NS_LOG_FUNCTION (macStats << path);
  ...
  macStats->UlScheduling (cellId, imsi, frameNo, subframeNo, rnti, mcs, size, componentCarrierId,repetition);
}
