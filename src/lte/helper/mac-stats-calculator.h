...
public:
  ...
  void UlScheduling (uint16_t cellId, uint64_t imsi,uint32_t frameNo, uint32_t subframeNo,
                     uint16_t rnti, uint8_t mcsTb, uint16_t sizeTb, uint8_t componentCarrierId, uint32_t repetition=1);
  ...
  static void UlSchedulingCallback (Ptr<MacStatsCalculator> macStats, std::string path,
                             ...
                             uint8_t mcs, uint16_t size, uint8_t componentCarrierId, uint32_t repetition=1);
