...
main (int argc, char *argv[])
{
  ...
  cmd.AddValue("maxmcs", "Maximum MCS", maxmcs); //<TODO: To connect maxmcs to lte-amc.cc>
  cmd.Parse(argc, argv);
  ...
  for (uint32_t u = 0; u < ueNodes.GetN (); ++u)
    {
      ...
      //ulClient.SetAttribute ("EnableRandom", UintegerValue (1));
      //ulClient.SetAttribute ("Percent", UintegerValue (100));

      UdpClientHelper client (ueIpIface.GetAddress (u), otherPort);
      client.SetAttribute ("Interval", TimeValue (MilliSeconds(interPacketIntervalul)));
      client.SetAttribute ("MaxPackets", UintegerValue(1000000));
      client.SetAttribute ("PacketSize", UintegerValue(packetsize));
      //client.SetAttribute ("EnableRandom", UintegerValue (1));
      //client.SetAttribute ("Percent", UintegerValue (100));
