...
main (int argc, char *argv[])
{
  ...
  */
  ...
  {
      building = CreateObject<Building> ();
      building->SetBoundaries (Box (3000 + i*buildingOffset, 3000 + i*buildingOffset + buildingWidth,
                                    -200, 100 + 200,
                                    0.0, nFloors* roomHeight));
      building->SetBuildingType (Building::Residential);
  ...
  {
    building = CreateObject<Building> ();
    building->SetBoundaries (Box (1800 + i*200, 1800 + i*200 + buildingWidth,
                                  -3200, -2900 + 100,
                                  0.0, nFloors* roomHeight));
    building->SetBuildingType (Building::Residential);
  ...
  {
    building = CreateObject<Building> ();
    building->SetBoundaries (Box (-1200 + i*200, -1200 + i*200 + buildingWidth,
                                  -3200, -2900 + 100,
                                  0.0, nFloors* roomHeight));
    building->SetBuildingType (Building::Residential);
    building->SetExtWallsType (Building::ConcreteWithWindows);
  ...
  {
    for (int i = 0; i < (numberOfUeNodes/(1*1/*numberOfEnbNodes*/)); i++)
      uePositionAlloc->Add (Vector ((enbX[n][0] + i*ueDistance+shift), enbY[n][0] + 0, nodeHeight));  // edgeUE1-100
    //for (int i = 0; i < (numberOfUeNodes/(sectorNum*1/*numberOfEnbNodes*/)); i++)
  ...
  for (uint32_t u = 0; u < ueNodes.GetN (); ++u)
    {
      ...
      ulClient.SetAttribute ("PacketSize", UintegerValue (12));
