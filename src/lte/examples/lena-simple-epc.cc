          ...
          if (uedev)
            {
              ...
              outFile << "set label \"" /*<< uedev->GetImsi ()*/
                      << "\" at " << pos.x << "," << 1*pos.y << " left font \"Helvetica,4\" textcolor rgb \"black\" front point pt 7 ps 1 lc rgb \"grey\" offset 0,0"
...
main (int argc, char *argv[])
{
  ...
  double simTime = 5;
  double appTrafficStartTime = 2.000;
  double appTrafficStopTime = 1000.0;
  double distance = 7000.0;
  bool useCa = false;
  double nodeHeight = 1.5;
  int maxPackets = 1;
  int sectorNum = 1;
  double ueDistance = 50;//f
  uint32_t buildingWidth = 100;
  uint8_t DlBandwidth = 100;//25;
  uint8_t UlBandwidth = 100;//25;
  UintegerValue uintegerValue;
  double interPacketInterval = 10000000;//33000//20,000,000;//900,000,000
...
//OPTION #2 HATA
 lteHelper->SetAttribute ("PathlossModel", StringValue ("ns3::OkumuraHataPropagationLossModel"));
  ...
  // ifTraceFile.open ("../../src/lte/model/fading-traces/fading_trace_EPA_0kmph.fad", std::ifstream::in);
  // if (ifTraceFile.good ())
  //   {
  //     // script launched by test.py
  //     lteHelper->SetFadingModelAttribute ("TraceFilename", StringValue ("../../src/lte/model/fading-traces/fading_trace_EPA_0kmph.fad"));
  //   }
  // else
  //   {
  ...
  // // - 100 RB
  // lteHelper->SetFadingModelAttribute ("TraceLength", TimeValue (Seconds (20.0)));
  // lteHelper->SetFadingModelAttribute ("SamplesNum", UintegerValue (10000));
  // lteHelper->SetFadingModelAttribute ("WindowSize", TimeValue (Seconds (0.5)));
  // lteHelper->SetFadingModelAttribute ("RbNum", UintegerValue (100));
  //===========================================================================

  // Geometry of the scenario (in meters) - linear
  // Assume squared building
  
  // uint32_t nEnbPerFloor = 1;
  // double roomHeight = 2;
  // uint32_t nFloors = 1;
  // uint32_t buildingOffset = 100;
  // uint32_t shiftb = 0;
  ...
  //     building->SetBoundaries (Box (shiftb + i*buildingOffset, 0 + i*buildingOffset + buildingWidth,
  //                                   -200, 100 + 200,
...
// Geometry of the scenario (in meters) - random
  // Assume squared building
  ...
  double roomHeight = 4;
  uint32_t nFloors = 10;
  uint32_t buildingOffset = 10;
  uint32_t shiftb = 0;
  uint32_t nRooms = std::ceil (std::sqrt (nEnbPerFloor));
  Ptr<Building> building;
Ptr<SequentialRandomVariable> fd = CreateObject<SequentialRandomVariable> ();
 // Ptr<UniformRandomVariable> fd = CreateObject<UniformRandomVariable> ();
  
  //fd->SetAttribute ("Consecutive", IntegerValue (2));
 Ptr<SequentialRandomVariable> fe = CreateObject<SequentialRandomVariable> ();
 // Ptr<UniformRandomVariable> fd = CreateObject<UniformRandomVariable> ();



  // for(int i = 0; i < 1600; i++)
  // { if((i<=90)||((i>100)&&(i<190))||((i>200)&&(i<290))||((i>300)&&(i<390))||
  //   ((i>400)&&(i<490))||((i>500)&&(i<590))||((i>600)&&(i<690))||((i>700)&&(i<790))||((i>800)&&(i<890))||((i>900)&&(i<990))||
  //   ((i>1000)&&(i<1090))||((i>1200)&&(i<1290))||((i>1300)&&(i<1390))||((i>1400)&&(i<1490)))
  //   { 
  //     fd->SetAttribute ("Min", DoubleValue (-3000.0));
  //     fd->SetAttribute ("Max", DoubleValue (0.0));
  //     fd->SetAttribute ("Increment", StringValue("ns3::UniformRandomVariable[Min=500.0|Max=1000.0]"));
  //     fe->SetAttribute ("Min", DoubleValue (0.0));
  //     fe->SetAttribute ("Max", DoubleValue (3000.0));
  //     fe->SetAttribute ("Increment", StringValue("ns3::UniformRandomVariable[Min=500.0|Max=1000.0]"));
  //    }
  //    else
  //         { 
  //     fd->SetAttribute ("Min", DoubleValue (-5000.0));
  //     fd->SetAttribute ("Max", DoubleValue (-3000.0));
  //     fd->SetAttribute ("Increment", StringValue("ns3::UniformRandomVariable[Min=500.0|Max=1000.0]"));
  //     fe->SetAttribute ("Min", DoubleValue (3000.0));
  //     fe->SetAttribute ("Max", DoubleValue (5000.0));
  //     fe->SetAttribute ("Increment", StringValue("ns3::UniformRandomVariable[Min=500.0|Max=1000.0]"));
  //    }
  //   double x=0.0;
  //   double y =0.0;
  ...
  //   std::cout << i << " x "<<x << " y " << y<< "\n";
  //     building = CreateObject<Building> ();
  //     // building->SetBoundaries (Box (shiftb + i*buildingOffset+x, 0 + i*buildingOffset + buildingWidth+x,-200+y, 100 + 200+y,
  //     //                               0.0, nFloors* roomHeight));

  //     building->SetBoundaries (Box ( x+i*buildingOffset, buildingWidth+x+i*buildingOffset,y,500+y,
  //                                   0.0, nFloors* roomHeight));
  //     building->SetBuildingType (Building::Residential);
  //     building->SetExtWallsType (Building::ConcreteWithoutWindows);//ConcreteWithWindows//StoneBlocks//ConcreteWithoutWindows
  //     building->SetNFloors (nFloors);
 ...
 //===========================================================================
   ...
   //=====================================MOBILITY AND POSITION ALLOCATION===================================
  ...
  double enbX[7][3] = {{0, 0, 0},  {3001.5, 2998.5, 3000.0}, {1501.5, 1498.5, 1500.0},   {-2998.5, -3001.5, -3000.0},  {-1498.5, -1501.5, -1500.0},  {1501.5, 1498.5, 1500.0},       {-1498.5, -1501.5, -1500.0}};
...
//UE OPTION 1 LINEAR 
  ...
  {
    for (int i = 1; i < (numberOfUeNodes+1); i++)
       {
     if(i>70 && i<=100) 
        shift=shift+100;
      if(i>100)
...
//UE OPTION 3 RANDOM  
 ...
 //  Box macroUeBox = Box (-distance * 1.5, distance * 1.5, -distance * 1.5, distance * 1.5, 1.5, 1.5);
 ...
 //BuildingsHelper::MakeMobilityModelConsistent ();
  ...
  lteHelper->SetFfrAlgorithmType ("ns3::LteFrNoOpAlgorithm");//LteFrNoOpAlgorithm//LteFrHardAlgorithm//LteFrStrictAlgorithm;
  Config::SetDefault ("ns3::LteEnbPhy::TxPower", DoubleValue (eNbTxPower));
      ...
      for (uint32_t b = 0; b < 1; ++b)
        {
         ...
         // clientApps.Add (dlClientHelper.Install (remoteHost));
          ...
          ulClientHelper.SetAttribute ("MaxPackets", UintegerValue (maxPackets));
           ulClientHelper.SetAttribute ("PacketSize", UintegerValue (12));
          ulClientHelper.SetAttribute ("Interval", TimeValue(MicroSeconds(x->GetValue ())));
