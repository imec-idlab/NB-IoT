       ...
       ++rxInfoIterator)
    {
      ...
      for (std::set<Ptr<SpectrumPhy> >::const_iterator rxPhyIterator = rxInfoIterator->second.m_rxPhySet.begin ();
        ...
        {  double pathLossDb = 0;
          ...
          /* if((*(rxParams->psd))[0]==0)
              {std::cout <<  "POSITION TX " << txMobility->GetPosition () << " POSITION RX" << receiverMobility->GetPosition () << "\n";
               std::cout << pathLossDb << " rxParams " << *(rxParams->psd)<< "\n";}*/
